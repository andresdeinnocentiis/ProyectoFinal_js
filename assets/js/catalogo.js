...
class Catalogo {
    ...
    addToCart() {
        ...
        btnsAdd.forEach((btn) => {
            ...
            btn.addEventListener("click", (event) => {
                ...
                // Si la cantidad del producto es mayor a 0 voy a poder sumarlo:
                if (amount > 0) {
                    ...
                    let cantidad = document.querySelector(`#contador-${id}`)

                    // Chequeo si ya está agregado al carrito y si no está lo agrego:
                    if (!this.inCart(item.id)) {

                        // Agrego el item al objeto cart del script
                        this.cart.push(cartItem);

                        // Restablezco la cantidad en el catalogo a "0":
                        ...
                        Storage.updateAmountProd(cartItem)
                        // Restablezco la cantidad en el catalogo a "0":
                        cantidad.innerText = 0

                        // Actualizo la caja del producto en el cart:
                        this.updateImgInCart(cartItem)
                        // ACTUALIZO EL TOTAL A PAGAR
                        this.actualizarTotal();
                        // ACTUALIZO EL TOTAL (CANTIDAD) DE ITEMS (EL NRO. DE ITEMS EN EL ICONO DEL CARRITO EN EL NAV)
                        this.getTotalItems();
    ...
    updateImgInCart(cartItem) {
        let storItem = Storage.getProduct(cartItem.id)
        let id = cartItem.id;
        
        let boxProd = document.querySelector(`#item-${id}`)
      
        let boxCant = boxProd.querySelector('.cant')
        
        let boxPrecio = boxProd.querySelector('.precioCart')
        
        boxCant.innerHTML = storItem.amount
        boxPrecio.innerHTML = storItem.precio * boxCant.innerHTML
    }
